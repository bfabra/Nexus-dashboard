...
- **Hoy**: Vista diaria con tareas, franja de la semana (día seleccionable para ver resumen y tareas de ese día), vencimientos próximos (tareas, cursos, hilos X), tarjetas de resumen (proyectos, cursos, PNL, lectura), hábitos diarios (checkboxes por día), countdown vacaciones.
...
- **Diario**: Entradas por fecha (texto, mood, tags), planner emocional por bloques (preguntas, 500 palabras máx), guardar por bloque.
...
| **Service Worker** | `CACHE_NAME`: `mi-espacio-v3`. |
| **CSS** | `appearance: none` (estándar) junto a `-webkit-appearance: none` en inputs/select. |
