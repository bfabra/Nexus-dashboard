...
export function renderTodayTasks() {
  ...
  tasks.forEach(t => {
    ...
    d.innerHTML = `
      <div class="task-check ${t.done ? 'done' : ''}" data-idx="${gi}">${t.done ? '✓' : ''}</div>
      <div class="task-text">
        <div class="task-name ${t.done ? 'done' : ''}">${t.name}</div>
        <div class="task-meta">${t.prio === 'alta' ? '🔴' : t.prio === 'media' ? '🟡' : '🟢'} ${(t.prio || '').charAt(0).toUpperCase() + (t.prio || '').slice(1)}</div>
      </div>
      <span class="badge ${cats[t.cat] || 'badge-gold'}">${clab[t.cat] || t.cat}</span>
      <button type="button" class="btn btn-sm btn-ghost task-edit-btn" data-idx="${gi}" title="Editar tarea">✏️</button>
    `;
    d.querySelector('.task-check').onclick = () => { toggleTask(gi); renderWeekDaySummary(); };
    const editBtn = d.querySelector('.task-edit-btn');
    if (editBtn) editBtn.onclick = () => openTaskEdit(gi);
    c.appendChild(d);
  });
}
...
export function saveTask() {
  ...
  const idx = idxInput && idxInput.value !== '' ? parseInt(idxInput.value, 10) : -1;
  ...
  if (!isNaN(idx) && idx >= 0 && DATA.tasks[idx]) {
...
function openTaskEdit(idx) {
  ...
  if (titleEl) titleEl.textContent = '✏️ Editar Tarea';
  if (idxInput) idxInput.value = String(idx);
  if (name) name.value = t.name || '';
  if (cat) cat.value = t.cat || 'proyecto';
  if (prio) prio.value = t.prio || 'media';
  if (date) date.value = t.date || todayKey();
